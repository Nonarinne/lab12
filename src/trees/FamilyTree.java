...
public class FamilyTree
{
	...
	private void addLine(String line) throws TreeException
	{
		...
		if (colonIndex < 0)
			throw new TreeException("Line does not contain colon");
...
//		?? For each name in childrenArray, create a new node and add that node to parentNode.
		for (String childString : childrenArray) {
			parentNode.addChild(new TreeNode(childString));
		}
	}
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null)
			throw new TreeException("name1 isn't in tree"); //Throw a TreeException with a useful message
		TreeNode node2 = root.getNodeWithName(name2);		// node whose name is name2
		if (node2 == null)
			throw new TreeException("name2 isn't in tree");//Throw TreeException with a useful message
